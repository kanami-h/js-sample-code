...
function bob(){
    ...
    for(let talkID=0; talkID<=5; talkID++){
        ...
        switch(talkID){
            ...
            case 3: {function whatBobSays() {
                return whatBobSays = LOCAL_NEST_FUNCTION;
            }//関数`bob()`の中で関数`whatBobSays`を作成し、その中で宣言したものを利用する
            setDialog(whatBobSays(),talkID);
        }
            break;

            case 4: {//関数`bob()`内のfor文のブロックスコープにある、ID4の文字列が格納された変数`whatBobSays`から取得する
                let whatBobSays = LOCAL_FOR;
                setDialog(whatBobSays,talkID);
            }
            break;

            default: const obj ={//オブジェクトを作成し、ID5の文字列を返すオブジェクトメソッド`whatBobSays`を使用する
                func: LOCAL_OBJ,
